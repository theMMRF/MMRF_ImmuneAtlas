...
```{r message=FALSE, warning=FALSE}
saveRDS(file='/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/RData/part4_os_subcluster_comp_fulldf.RDS',subcluster_comp_fulldf)
saveRDS(file='/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/RData/part4_os_subcluster_all_fulldf.RDS',subcluster_all_fulldf)
```
...
mod.bi <- lrm(censos ~ d_dx_amm_iss_stage + d_dx_amm_age + d_pt_sex, subcluster_all_fulldf, x=TRUE,y=TRUE)
...
mod.bi <- lrm(censos ~ d_dx_amm_iss_stage + d_dx_amm_age + d_pt_sex + batch + site, subcluster_all_fulldf, x=TRUE,y=TRUE)
...
mod.bi <- lrm(censos ~ davies_based_risk + d_dx_amm_age + d_pt_sex + batch + site + d_dx_amm_iss_stage, subcluster_all_fulldf, x=TRUE, y=TRUE)
...
my_form_V3 <- list()
for(iii in my_subcluster_names){ my_form_V3[[iii]] <- as.formula(paste("censos ~ d_dx_amm_age + d_pt_sex + batch +", iii)) }

my_form_V4 <- list()
for(iii in my_subcluster_names){ my_form_V4[[iii]] <- as.formula(paste("censos ~ d_dx_amm_age + d_pt_sex + batch + site +", iii)) }

my_form_V5 <- list()
for(iii in my_subcluster_names){ my_form_V5[[iii]] <- as.formula(paste("censos ~ d_dx_amm_age + d_pt_sex + batch + site + d_dx_amm_iss_stage +", iii)) }

my_form_V6 <- list()
for(iii in my_subcluster_names){ my_form_V6[[iii]] <- as.formula(paste("censos ~ davies_based_risk + d_dx_amm_age + d_pt_sex + batch + site + d_dx_amm_iss_stage +", iii)) }
```
...
`
```{r message=FALSE, warning=FALSE}
b <- paste( my_subcluster_names[!my_subcluster_names %in% c('NkT.8','NkT.7')] ,'+')
...
z <- z[-c(1:3)] # manually selected values to remove covariates
```
...
used_features<-list(R1=Feat_Sel_R1_Features,
     R2=Feat_Sel_R2_Features,
     R3=Feat_Sel_R3_Features,
     R4=Feat_Sel_R4_Features)
```

```{r message=FALSE, warning=FALSE}
saveRDS(file = '/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/RData/pred_validation_list_OBJ_OS.rds',pred_validation_list)
saveRDS(file = '/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/RData/predictions_OBJ_OS.rds',predictions)
saveRDS(file = '/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/RData/labels_OBJ_OS.rds',labels)
saveRDS(file = '/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/RData/aucs_OBJ_OS.rds',aucs)
saveRDS(file = '/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/RData/used_features_OS.rds',used_features)
```

```{r message=FALSE, warning=FALSE}
pred_validation_list <- readRDS(file = '/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/RData/pred_validation_list_OBJ_OS.rds',)
predictions <- readRDS(file = '/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/RData/predictions_OBJ_OS.rds')
labels <- readRDS(file = '/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/RData/labels_OBJ_OS.rds')
aucs <- readRDS(file = '/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/RData/aucs_OBJ_OS.rds')
...
```{r message=FALSE, warning=FALSE}
predictions_list <- pred_validation_list
pred <- prediction(predictions, labels)
perf <- performance(pred,'tpr','fpr')
...
pdf(file='/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/figures/ROC_article_boostraping_model_type_OS.pdf',width = 5,height = 3)
ggplot(data=x[which(!is.na(x$type)),])+aes(x=x,y=y,color=type) + geom_line(show.legend = F,aes(group=model)) + theme_classic() + geom_smooth() + geom_abline(color='black') +
  theme(legend.position = 'right') + labs(x='False Positive Rate',y='True Positive Rate',title = 'Receiver Operating Characteristic Curve')
dev.off()
```
...
x$type[x$model %in% 'Feat_Sel_R1'] <- '32 SubC Imm.Atlas+CoVs'
x$type[x$model %in% 'Feat_Sel_R2'] <- '21 SubC Imm.Atlas+CoVs'
x$type[x$model %in% 'Feat_Sel_R3'] <- '20 SubC Imm.Atlas+CoVs'
x$type[x$model %in% 'Feat_Sel_R4'] <- '20 SubC Imm.Atlas+CoVs'
###
x$type <- factor(x$type,levels = c('Single','+Age','+Age+Sex','+Age+B*Site','+Age+B*Site+ISS','+Cyto+Age+B*Site+ISS','20 SubC Imm.Atlas+CoVs',
                                   '21 SubC Imm.Atlas+CoVs','32 SubC Imm.Atlas+CoVs','Full Imm.Atlas+CoVs'))
```
...
forest_model( coxph(Surv(ttcos, censos==1) ~ d_pt_sex + d_dx_amm_age + d_dx_amm_iss_stage + site + batch, data = tmp_df) )
```
...
x$simple_name[x$model %in% 'Feat_Sel_R2'] <- 'CYTO+IA[20]'
x$simple_name[x$model %in% 'Feat_Sel_R1'] <- 'CYTO+IA[32]'
...
```{r message=FALSE, warning=FALSE}
#predictions is part of predictions_list, hence a problem
...
tmp_df_V3$p <- as.numeric(predictions$mod_iss_V4) > median(as.numeric(predictions$mod_iss_V4))
...
write.csv(file='/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/RData/revised_extendeddata_fig8e_pfs.csv',dataForRDDPFS)
```
...
ggsurvplot( fit = survfit(Surv(ttcos, censos==1) ~ p, data = tmp_df_V3), 
            ...
            xlab = "Days", ylab = "OS Prob.",pval = TRUE,risk.table = TRUE,palette = c('black','#64B200')) +labs(title = 'ISS+CoVs')
```
...
```{r message=FALSE, warning=FALSE}
pdf(file = '/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/figures/km_labels_model_ISS_CoVs_OS.pdf',width = 3.85, height = 4.95)
ggsurvplot( fit = survfit(Surv(ttcos, censos==1) ~ p, data = tmp_df_V3), 
            type=c("kaplan-meier"),surv.median.line = 'hv',log.rank.weights= "survdiff",pval.method = TRUE,
            xlab = "Days", ylab = "OS Prob.",pval = TRUE,risk.table = TRUE,palette = c('black','#64B200')) +labs(title = 'ISS+CoVs')
dev.off()
...
#tmp_df_V3$p <- as.numeric(predictions_list$mod_iss_V4$predictions) > median(as.numeric(predictions_list$mod_iss_V4$predictions))
tmp_df_V3$p <- as.numeric(predictions$mod_cyto_V5) > median(as.numeric(predictions$mod_cyto_V5))
tmp_df_V3$p[tmp_df_V3$p == '1'] <- 'High'
tmp_df_V3$p[tmp_df_V3$p == '0'] <- 'Low'
```
...
```{r message=FALSE, warning=FALSE}
dataForRDDPFS <- data.frame(ttcpfs=tmp_df_V3$ttcpfs,censpfs=tmp_df_V3$censpfs,prediction=tmp_df_V3$p,sample_id=tmp_df_V3$sample_id)
write.csv(file='/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/RData/revised_extendeddata_fig8f_pfs.csv',dataForRDDPFS)
```
...
ggsurvplot( fit = survfit(Surv(ttcos, censos==1) ~ p, data = tmp_df_V3), 
            type=c("kaplan-meier"),surv.median.line = 'hv',log.rank.weights= "survdiff",pval.method = TRUE,
            xlab = "Days", ylab = "OS Prob.",pval = TRUE,risk.table = TRUE,palette = c('black','#00BD5C')) +labs(title = 'CYTO+CoVs')
```
...
```{r message=FALSE, warning=FALSE}
pdf(file = '/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/figures/km_labels_model_CYTO_CoVs_OS.pdf',width = 3.85, height = 4.95)
ggsurvplot( fit = survfit(Surv(ttcos, censos==1) ~ p, data = tmp_df_V3), 
            type=c("kaplan-meier"),surv.median.line = 'hv',log.rank.weights= "survdiff",pval.method = TRUE,
            xlab = "Days", ylab = "OS Prob.",pval = TRUE,risk.table = TRUE,palette = c('black','#00BD5C')) +labs(title = 'CYTO+CoVs')
dev.off()
...
tmp_df_V3$p <- as.numeric(predictions$Myeloid.8_V5) > median(as.numeric(predictions$Myeloid.8_V5))
tmp_df_V3$p[tmp_df_V3$p == '1'] <- 'High'
tmp_df_V3$p[tmp_df_V3$p == '0'] <- 'Low'
```
...
ggsurvplot( fit = survfit(Surv(ttcos, censos==1) ~ p, data = tmp_df_V3), 
            type=c("kaplan-meier"),surv.median.line = 'hv',log.rank.weights= "survdiff",pval.method = TRUE,
            xlab = "Days", ylab = "OS Prob.",pval = TRUE,risk.table = TRUE,palette = c('black','#00C1A7')) +labs(title = 'IA+CoVs')
```
...
```{r message=FALSE, warning=FALSE}
dataForRDDPFS <- data.frame(ttcpfs=tmp_df_V3$ttcpfs,censpfs=tmp_df_V3$censpfs,prediction=tmp_df_V3$p,sample_id=tmp_df_V3$sample_id)
write.csv(file='/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/RData/revised_extendeddata_fig8g_pfs.csv',dataForRDDPFS)
```
...
```{r message=FALSE, warning=FALSE}
pdf(file = '/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/figures/km_labels_model_IA_CoVs_OS.pdf',width = 3.85, height = 4.95)
ggsurvplot( fit = survfit(Surv(ttcos, censos==1) ~ p, data = tmp_df_V3), 
            type=c("kaplan-meier"),surv.median.line = 'hv',log.rank.weights= "survdiff",pval.method = TRUE,
            xlab = "Days", ylab = "OS Prob.",pval = TRUE,risk.table = TRUE,palette = c('black','#00C1A7')) +labs(title = 'IA+CoVs')
dev.off()
...
tmp_df_V3$p <- as.numeric(predictions$Myeloid.8_V6) > median(as.numeric(predictions$Myeloid.8_V6))
tmp_df_V3$p <- as.numeric(predictions$NkT.8_V6) > median(as.numeric(predictions$NkT.8_V6))
tmp_df_V3$p[tmp_df_V3$p == '1'] <- 'High'
tmp_df_V3$p[tmp_df_V3$p == '0'] <- 'Low'
```
...
ggsurvplot( fit = survfit(Surv(ttcos, censos==1) ~ p, data = tmp_df_V3), 
            type=c("kaplan-meier"),surv.median.line = 'hv',log.rank.weights= "survdiff",pval.method = TRUE,
            xlab = "Days", ylab = "OS Prob.",pval = TRUE,risk.table = TRUE,palette = c('black','#00BADE')) +labs(title = 'CYTO+IA+CoVs')
```
...
```{r message=FALSE, warning=FALSE}
pdf(file = '/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/figures/km_labels_model_IA_CYTO_CoVs_OS.pdf',width = 3.85, height = 4.95)
ggsurvplot( fit = survfit(Surv(ttcos, censos==1) ~ p, data = tmp_df_V3), 
            type=c("kaplan-meier"),surv.median.line = 'hv',log.rank.weights= "survdiff",pval.method = TRUE,
            xlab = "Days", ylab = "OS Prob.",pval = TRUE,risk.table = TRUE,palette = c('black','#00BADE')) +labs(title = 'CYTO+IA+CoVs')
dev.off()
...
ggsurvplot( fit = survfit(Surv(ttcos, censos==1) ~ p, data = tmp_df_V3), 
            type=c("kaplan-meier"),surv.median.line = 'hv',log.rank.weights= "survdiff",pval.method = TRUE,
            xlab = "Days", ylab = "OS Prob.",pval = TRUE,risk.table = TRUE,palette = c('black','#B385FF')) +labs(title = 'CYTO+IA[20]')
```
...
```{r message=FALSE, warning=FALSE}
pdf(file = '/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/figures/km_labels_model_IA_CYTO_CoVs_v0_OS.pdf',width = 3.85, height = 4.95)
ggsurvplot( fit = survfit(Surv(ttcos, censos==1) ~ p, data = tmp_df_V3), 
            type=c("kaplan-meier"),surv.median.line = 'hv',log.rank.weights= "survdiff",pval.method = TRUE,
            xlab = "Days", ylab = "OS Prob.",pval = TRUE,risk.table = TRUE,palette = c('black','#B385FF')) +labs(title = 'CYTO+IA[20]')
dev.off()
...
tmp_df_V3$p <- as.numeric(predictions$Feat_Sel_R2) > median(as.numeric(predictions$Feat_Sel_R2))
tmp_df_V3$p[tmp_df_V3$p == '1'] <- 'High'
tmp_df_V3$p[tmp_df_V3$p == '0'] <- 'Low'
```
...
ggsurvplot( fit = survfit(Surv(ttcos, censos==1) ~ p, data = tmp_df_V3), 
            type=c("kaplan-meier"),surv.median.line = 'hv',log.rank.weights= "survdiff",pval.method = TRUE,
            xlab = "Days", ylab = "OS Prob.",pval = TRUE,risk.table = TRUE,palette = c('black','#EF67EB')) +labs(title = 'CYTO+IA[20]')
```

```{r message=FALSE, warning=FALSE}
dataForRDDPFS <- data.frame(ttcpfs=tmp_df_V3$ttcpfs,censpfs=tmp_df_V3$censpfs,prediction=tmp_df_V3$p,sample_id=tmp_df_V3$sample_id)
write.csv(file='/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/RData/revised_extendeddata_fig8h_pfs.csv',dataForRDDPFS)
```
...
```{r message=FALSE, warning=FALSE}
pdf(file = '/Users/gonzae34/Documents/projects_gnjatic/MMRF_projects/immune_atlas/figures/km_labels_model_IA_CYTO_20_CoVs_v1_OS.pdf',width = 3.85, height = 4.95)
ggsurvplot( fit = survfit(Surv(ttcos, censos==1) ~ p, data = tmp_df_V3), 
            type=c("kaplan-meier"),surv.median.line = 'hv',log.rank.weights= "survdiff",pval.method = TRUE,
            xlab = "Days", ylab = "OS Prob.",pval = TRUE,risk.table = TRUE,palette = c('black','#EF67EB')) +labs(title = 'CYTO+IA[20]')
dev.off()
...
ggsurvplot( fit = survfit(Surv(ttcos, censos==1) ~ p, data = tmp_df_V3), 
            type=c("kaplan-meier"),surv.median.line = 'hv',log.rank.weights= "survdiff",pval.method = TRUE,
            xlab = "Days", ylab = "OS Prob.",pval = TRUE,risk.table = TRUE,palette = c('black','#FF63B6')) +labs(title = 'CYTO+IA[FULL]')
```
